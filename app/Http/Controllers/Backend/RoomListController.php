...
class RoomListController extends Controller
{
    ...
    public function ViewRoomList(){
        ...
        ->orderBy('rooms_type_id','asc')
        ->leftjoin('room_types','room_types.id','room_numbers.rooms_type_id')
        ...
        ->select(
            ...
            'bookings.status as booking_stat',
    ...
    public function StoreRoomList(Request $request){
        ...
        if ($request->check_in == $request->check_out) {
            $request->flash();
            $notification = array(
                'message' =>'Vous avez entrer les meme dates',
                'alert-type' => 'error'
            );
        ...
        if ($request->available_room < $request->number_of_rooms) {
            $notification = array(
                'message' =>'Nombre de chambre superieure à la disponiblité',
                'alert-type' => 'error'
            );
            return  redirect()->back()->with($notification);
        }
      ...
      if ($room->room_capacity < $request->number_of_person) {
        $notification = array(
            'message' =>'Le nombre de personne est superieur au capacité de la chambre',
            'alert-type' => 'error'
        );
        return  redirect()->back()->with($notification);
      }
        ...
        $notification = array(
            'message' =>'Réservation Ajouter avec succes',
            'alert-type' => 'success'
        );
        return  redirect()->back()->with($notification);
